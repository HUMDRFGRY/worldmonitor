...
Basemap tile configuration lives in `src/config/basemap.ts`. See [Map Engine](./map-engine.mdx) for full details on tile providers (PMTiles, OpenFreeMap, CARTO), themes, and fallback behavior.
