...
See [Adding Endpoints](./adding-endpoints.mdx) for the full proto workflow.
...
World Monitor is licensed under the [GNU Affero General Public License v3.0 (AGPL-3.0)](./license.mdx) for non-commercial use. Commercial use requires a [separate license](./license.mdx#commercial-use-requires-a-separate-license). See the [License](./license.mdx) page for full details on what this means in practice.
