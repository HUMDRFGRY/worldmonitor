...
Country boundaries, boundary overrides, and the geocoding service are documented in [Maps & Geocoding](./maps-and-geocoding.mdx). All large static files are served from R2 CDN via `maps.worldmonitor.app`.
