...
- **New here?** Start with [Getting Started](./getting-started.mdx) for installation and setup
- **Want to understand the system?** Read [Architecture](./architecture.mdx) for how the pieces fit together
- **Looking for specific features?** See [Features & Interface](./features.mdx) for the full capability list
- **Interested in the data?** Check [Data Sources](./data-sources.mdx) for all 31+ sources and collection methods
- **Want to contribute?** Read [Contributing](./contributing.mdx) for code style and PR process
- **Building on the API?** Browse the [API Reference](./api/) for all 24 typed services

## License

World Monitor is open source under [AGPL-3.0](./license.mdx). Free for personal, educational, and research use. Commercial use requires a [separate license](./license.mdx#commercial-use-requires-a-separate-license).
